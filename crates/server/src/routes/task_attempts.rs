...
pub struct CreateFollowUpAttempt {
    ...
    // Optional retry settings (merge retry endpoint into follow-up)
...
pub async fn follow_up(
    ...
    if let Some(proc_id) = payload.retry_process_id {
        ...
        if perform_git_reset && let Some(target_oid) = &target_before_oid {
        ...
        // Best-effort: clear any retry draft for this attempt (UI should switch back to follow-up box)
        let _ = Draft::clear_after_send(pool, task_attempt.id, DraftType::Retry).await;
        // Immediately notify WS clients; SQLite hook on DELETE cannot recover attempt_id reliably
        deployment
            .events()
    ...
    // - If this was a retry send, the retry draft has already been cleared above; preserve follow-up draft state.
    // - Otherwise, clear the follow-up draft to avoid stale UI after manual send.
...
pub fn router(deployment: &DeploymentImpl) -> Router<DeploymentImpl> {
    let task_attempt_id_router = Router::new()
        ...
        // Unified draft endpoints (delegated to task_attempt_drafts)
        .route(
            "/draft",
            get(task_attempt_drafts::get_draft)
                .put(task_attempt_drafts::save_draft)
                .delete(task_attempt_drafts::delete_draft),
        )
        .route("/draft/queue", post(task_attempt_drafts::set_draft_queue))
