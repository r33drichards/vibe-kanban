...
/**
 * Manual sort - sort by custom order stored in localStorage.
 * Allows users to drag and drop tasks in any order they want.
 */
export function manualSort(
  tasks: TaskWithAttemptStatus[],
  projectId: string
): TaskWithAttemptStatus[] {
  const storageKey = `task-manual-order-${projectId}`;
  const storedOrder = localStorage.getItem(storageKey);

  if (!storedOrder) {
    // No manual order set yet, return tasks as-is
    return tasks;
  }

  try {
    const orderMap: Record<string, number> = JSON.parse(storedOrder);

    // Sort tasks by their manual order, tasks without an order go to the end
    ...
    console.error('Failed to parse manual task order:', error);
    return tasks;
  }
}
 ...
 */
export function saveManualOrder(taskIds: string[], projectId: string): void {
  const storageKey = `task-manual-order-${projectId}`;
